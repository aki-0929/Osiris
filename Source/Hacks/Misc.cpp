...
void Misc::watermark() noexcept
{
    ...
    ImGui::Text("Siriso | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Siriso\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity->getPlayerName().c_str(), color, votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Siriso\u2022 %c%s\x01 call vote (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity->getPlayerName().c_str(), voteName(voteType));
}

void Misc::onVotePass() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Siriso\u2022\x01 Vote\x06 PASSED");
}

void Misc::onVoteFailed() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022Siriso\u2022\x01 Vote\x07 FAILED");
}
